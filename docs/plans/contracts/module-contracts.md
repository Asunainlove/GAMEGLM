...
| `res://scenes/battle.tscn` | `Battle` (Node2D, script `res://src/encounters/battle_scene.gd`) | `Tracks` (Node2D; wired units `luoxian_fighter`/`misa_weaver`/`drift_swarmling`/`shard_husk`/`veinwarden_echo` → AnimatedSprite2D 8-frame via AssetAdapter; others graybox), `UI` (CanvasLayer) |
...
- **叙事** `class_name EventRunner`（WP08）：`load_events_from(dir: String) -> AppResult`；`available_events(events: Array, state: Dictionary) -> Array[String]`；`next_step(event_def: Dictionary, step_index: int) -> Dictionary`（返回步骤或 `{}`）；`choose_option(state: Dictionary, event_def: Dictionary, step: Dictionary, option: Dictionary, store: Object = null) -> AppResult`（仅直接施加 main 既有操作 `set_flag`/`add_item`/`remove_item`；`relation_delta` 进入 `value.deferred_ops`（op 字典数组，形如 `{"op": "set_relationship", "char_id": ..., "dim": ..., "value": ...}`），由 WP04 合并后的集成层施加；事件完成标记写 flag `event_<id>_done`）。
...
- **推进** `class_name Progression`（WP14）：`due_event(state: Dictionary) -> String`（按契约 §7 事件顺序返回应触发的事件 id 或 `""`）；`react(state: Dictionary, signal_name: String, payload: Dictionary, store: Object = null) -> AppResult`（signal ∈ `mined`/`built`/`event_completed`/`encounter_won`：写对应一次性 flag、选择链推进、Boss 条件与遭遇 due flag；对 WP04 新操作经注入 store 的未类型化 patch 调用）；`world_response_ops(state: Dictionary, trigger: String) -> Array[Dictionary]`（纯函数：返回世界回应 patch 操作描述，如 `station_mode_exploit` → flag `world_response_exploited`、`policy_extraction_quota` → flag `boss_condition_escalated`）。
