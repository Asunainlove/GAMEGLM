...
func test_p02_registers_exactly_the_frozen_autoload_set() -> void:
	...
	autoload_names.sort()
	assert_eq(autoload_names, ["ContentDB", "GameState", "SaveService"])
	...
	assert_eq(ProjectSettings.get_setting("autoload/ContentDB"), "*res://src/content/content_db.gd")
