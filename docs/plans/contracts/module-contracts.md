...
| `res://scenes/battle.tscn` | `Battle` (Node2D, script `res://src/encounters/battle_scene.gd`) | `Tracks` (Node2D; wired units `luoxian_fighter`/`misa_weaver`/`drift_swarmling`/`shard_husk` → AnimatedSprite2D 8-frame via AssetAdapter; others graybox), `UI` (CanvasLayer) |
